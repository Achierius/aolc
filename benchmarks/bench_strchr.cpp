...
static void BM_strchr(benchmark::State& state, size_t len) {
    ...
    std::uniform_int_distribution<char> char_dist(1, 126); 
  ...
  //tok = str_buffer[index_dist(index_gen)];
    tok = 127;

    for (auto _ : state) {
            // Only the contents of this loop get benchmarked
#ifdef __BENCH_AOLC__
            benchmark::DoNotOptimize(_strchr(str_buffer, tok));
#elif  __BENCH_GLIBC__
            benchmark::DoNotOptimize(strchr(str_buffer, tok));
#elif  __BENCH_MUSL__
...
int main(int argc, char** argv) {
    auto NameTest = [](int len){
        return "strchr_Len" + std::to_string(len);
    };
    std::array<int, 9> test_lengths = {4, 16, 64, 256, 4096, 16384, 1, 100, 10000};
    for (auto len : test_lengths) {
        benchmark::RegisterBenchmark(NameTest(len).c_str(), BM_strchr, len);
    }
