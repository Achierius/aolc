...
static void BM_memcpy_NBytes(benchmark::State& state, size_t len, size_t src_off, size_t dst_off) {
...
#ifdef __BENCH_AOLC__
            benchmark::DoNotOptimize(_memcpy(dst, src, len));
#elif  __BENCH_GLIBC__
            benchmark::DoNotOptimize(memcpy(dst, src, len));
#elif  __BENCH_MUSL__
...
int main(int argc, char** argv) {
    std::string str_base = "memcpy-Len";
    std::array<int, 13> test_lengths = {4, 8, 16, 32, 64, 256, 4096, 16384, 1, 10, 100, 1000, 10000};
    char test_char = '~';
    for (auto len : test_lengths) {
        benchmark::RegisterBenchmark((str_base + std::to_string(len)).c_str(), BM_memcpy_NBytes, len, 0, 0);
    }
