...
static void BM_strcpy_NBytes(benchmark::State& state, size_t len, size_t src_off, size_t dst_off) {
...
#ifdef __BENCH_AOLC__
            benchmark::DoNotOptimize(_strcpy(dst, src));
...
int main(int argc, char** argv) {
    std::string str_base = "strcpy-Len";
    std::array<int, 13> test_lengths = {4, 8, 16, 32, 64, 256, 4096, 16384, 1, 10, 100, 1000, 10000};
    char test_char = '~';
    for (auto len : test_lengths) {
        benchmark::RegisterBenchmark((str_base + std::to_string(len)).c_str(), BM_strcpy_NBytes, len, 0, 0);
    }
